...
export class SingerAdminEventProxy {
   ...
   acceptRegistration(eventId: string, eventRegistration: string): Observable<RegistrationStatus> {
      return this.apiService
         .post(`api/event/${eventId}/registrations/${eventRegistration}/accept`)
         .pipe(map(res => res));
   }

   rejectRegistration(eventId: string, eventRegistration: string): Observable<RegistrationStatus> {
      return this.apiService
   ...
   updateDaycareLocation(
      ...
      return this.apiService
         .put(`api/event/${eventId}/registrations/${eventRegistrationId}/location`, daycareLocationId)
         .pipe(map(res => res));
   }
}
